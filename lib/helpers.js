...
const request = async (url, options) => {
  ...
  // Set initial attempt count and max attempts
  let attempts = 0;
  let maxAttempts = 50;

  // Iterate through API tokens
  for (let i = 0; i < API_TOKEN.length; i++) {
    const token = API_TOKEN[i];

    while (attempts < maxAttempts) {
      try {
        // Construct headers with the current token
        const headers = {
        ...
        attempts++;
        console.warn(
          `An error occurred while making a web request: "${error}", retrying. Attempt ${attempts} of ${maxAttempts}.`
        );

        if (attempts >= maxAttempts) {
          // If it's the last token, throw an error
          if (i === API_TOKEN.length - 1) {
            ...
            console.warn("Moving to the next API token.");
            break; // Exit the inner loop and move to the next token
          }
        }
    ...
    // Reset attempts for the next token
    attempts = 0;
  }

  // If all tokens are exhausted, throw an error
  throw new Error("All API tokens failed to make the request.");
};
