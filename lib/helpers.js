...
const request = async (url, options) => {
  ...
  const API_TOKENS = ['API_TOKEN', 'API_TOKEN_1', 'API_TOKEN_2', 'API_TOKEN_3'];
  ...
  for (let token of API_TOKENS) {
    if (!token) continue; // Skip if the token is undefined or empty

    const headers = {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json",
      ...(API_KEY ? { "X-Auth-Email": ACCOUNT_EMAIL, "X-Auth-Key": API_KEY } : {})
    };
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        attempts++;
        console.warn(`An error occurred while making a web request with ${token}: "${error}", retrying. Attempt ${attempts} of ${maxAttempts}.`);
        if (attempts >= maxAttempts) {
          // Send a message to the Discord webhook if it exists
          await notifyWebhook(`An HTTP error has occurred (${response ? response.status : "unknown status"}) while making a web request with ${token}. Please check the logs for further details.`);
          // Do not throw an error yet, try the next token
        }
        await wait(5000); // Wait for 5 seconds before retrying
      }
    }
    attempts = 0; // Reset attempts for the next token
  }
