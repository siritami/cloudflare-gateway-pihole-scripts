...
export const API_TOKEN = process.env.CLOUDFLARE_API_TOKEN
  ? process.env.CLOUDFLARE_API_TOKEN.split('\n').map(token => token.trim()).filter(Boolean)
  : [];

// ... (other code, including API_KEY, ACCOUNT_ID, ACCOUNT_EMAIL, notifyWebhook)

const request = async (url, options) => {
  if (!((API_TOKEN && API_TOKEN.length > 0) || API_KEY) || !ACCOUNT_ID) {
    throw new Error(
      "The following secrets are required: CLOUDFLARE_API_TOKEN (or CLOUDFLARE_API_KEY), CLOUDFLARE_ACCOUNT_ID"
    );
  }

  const headers = API_TOKEN.length > 0
    ? {
        Authorization: '', // Will be set dynamically below
  ...
  */
  ...
  while (attempts < maxAttempts) {
    try {
      // Set the Authorization header with the current API token
      if (API_TOKEN.length > 0) {
        headers.Authorization = `Bearer ${API_TOKEN[tokenIndex]}`;
      }
      ...
      attempts++;
      console.warn(
        `An error occured while making a web request: "${error}", retrying. Attempt ${attempts} of ${maxAttempts}.`
      );

      // Cycle through API tokens before retrying
      tokenIndex = (tokenIndex + 1) % API_TOKEN.length;
